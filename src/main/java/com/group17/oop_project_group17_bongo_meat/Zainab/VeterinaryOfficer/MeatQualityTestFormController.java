...
public class MeatQualityTestFormController {
    ...
    public void generateReportButtonOnAction(ActionEvent actionEvent) {
        ...
        try {
            ...
            document.add(new Paragraph("Batch ID        : " + r.getBatchId()));
            document.add(new Paragraph("Cut Type        : " + r.getCutType()));
            document.add(new Paragraph("Weight          : " + r.getWeight() + " kg"));
            document.add(new Paragraph("Processing Date : " + r.getProcessingDate()));
            document.add(new Paragraph("Storage Time    : " + r.getStorageTime()));
            document.add(new Paragraph("Temperature     : " + r.getTemperature()));
            document.add(new Paragraph(""));
            document.add(new Paragraph("Vet ID          : " + r.getVetId()));
            document.add(new Paragraph("Vet License     : " + r.getVetLicense()));
            document.add(new Paragraph("Test DateTime   : " + r.getTestDateTime()));
            document.add(new Paragraph(""));
            document.add(new Paragraph("pH Value        : " + r.getpHValue()));
            document.add(new Paragraph("Microbial Count : " + r.getMicrobialCount()));
            if (r.getMoisturePercent() != null) {
                document.add(new Paragraph("Moisture %      : " + r.getMoisturePercent()));
            }
            document.add(new Paragraph("Storage Temp    : " + r.getStorageTemp() + " °C"));
            document.add(new Paragraph("Color Score     : " + r.getColorScore()));
            document.add(new Paragraph("Odor Evaluation : " + r.getOdorEvaluation()));
            document.add(new Paragraph("Foreign Matter  : " + r.getForeignMatterCheck()));
            document.add(new Paragraph("Remarks         : " + r.getRemarks()));
            document.add(new Paragraph(""));
            document.add(new Paragraph("Decision        : " + r.getEvaluationResult()));
            document.add(new Paragraph(""));
