...
public class NutritionFeedingRecordController {
    ...
    public void generateReportButtonOnAction(ActionEvent actionEvent) {
        if (selectedRecord == null) {
            ...
            reportGenerateMessageLabel.setText("Select a batch first to generate report.");
            return;
        }

        if (!"Scheduled".equalsIgnoreCase(selectedRecord.getStatus())) {
            reportGenerateMessageLabel.setStyle("-fx-text-fill: red;");
            reportGenerateMessageLabel.setText("Please save and assign schedule before generating report.");
            return;
        }

        FileChooser chooser = new FileChooser();
        chooser.setTitle("Save Feeding Schedule Report");
        ...
        try {
            ...
            doc.add(new Paragraph("Feeding Schedule Report"));
            doc.add(new Paragraph("------------------------------"));
            doc.add(new Paragraph("Animal ID: " + selectedRecord.getAnimalId()));
            doc.add(new Paragraph("Type: " + selectedRecord.getType()));
            doc.add(new Paragraph("Weight: " + selectedRecord.getWeight() + " kg"));
            doc.add(new Paragraph("Feed Type: " + selectedRecord.getFeedType()));
            doc.add(new Paragraph("Quantity: " + selectedRecord.getQuantity()));
            doc.add(new Paragraph("Feeding Time: " + selectedRecord.getFeedingTime()));
            doc.add(new Paragraph("Notes: " + selectedRecord.getNotes()));
            doc.add(new Paragraph("Status: " + selectedRecord.getStatus()));
            doc.add(new Paragraph("Generated By: Veterinary Officer"));
